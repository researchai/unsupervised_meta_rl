...
def run_task(v):
    ...
    # assert len(EASY_MODE_CLS_DICT.keys()) == N_TASKS
    # mt10 = MultiClassMultiTaskEnv(
    ...
    # goals_dict = {
    #     t: [e.goal.copy()]
    #     for t, e in zip(mt10._task_names, mt10._task_envs)}
    # mt10.discretize_goal_space(goals_dict)
    # # reach, push, pickplace are different
    # mt10._task_envs[0].obs_type = 'reach-v1'
    # mt10._task_envs[1].obs_type = 'pickplace-v1'
    # mt10._task_envs[2].obs_type = 'push-v1'
    # mt10._task_envs[3].obs_type = 'reach-v1'
    # mt10._task_envs[4].obs_type = 'pickplace-v1'
    # mt10._task_envs[5].obs_type = 'push-v1'
    # mt10._task_envs[0].goal = np.array([-0.1, 0.8, 0.2])
    # mt10._task_envs[1].goal = np.array([0.1, 0.8, 0.2])
    # mt10._task_envs[2].goal = np.array([0.1, 0.8, 0.02])
    # mt10._task_envs[3].goal = np.array([-0.05, 0.8, 0.2])
    # mt10._task_envs[4].goal = np.array([0.05, 0.8, 0.2])
    # mt10._task_envs[5].goal = np.array([0.05, 0.8, 0.015])
    from metaworld.benchmarks import MT10

    mt10 = MT10.get_train_tasks()
...
run_experiment(
    ...
    exp_prefix='mt10_seed{}'.format(SEED),
    n_parallel=4,
    seed=SEED,
    variant=config,
    snapshot_mode='last',
