...
from garage.tf.regressors.gaussian_gru_regressor_model import (
    GaussianGRURegressorModel)


class GaussianGRURegressor(StochasticRegressor):
    ...
    def __init__(self,
        ...
        self.model = GaussianGRURegressorModel(
    ...
    def _initialize(self):
        input_var = tf.compat.v1.placeholder(tf.float32,
                                          shape=(None, None, ) + self._input_shape)
        ...
        with tf.compat.v1.variable_scope(self._variable_scope):
            self.model.build(input_var, step_input_var, step_hidden_var)
            ys_var = tf.compat.v1.placeholder(dtype=tf.float32,
                                              name='ys',
                                              shape=(None, None, self._output_dim))
    ...
    def fit(self, xs, ys):
        ...
        if self._normalize_inputs:
            ...
            self.model.networks['default'].x_mean.load(
                np.mean(np.mean(xs, axis=0), axis=0, keepdims=True))
            self.model.networks['default'].x_std.load(
                np.std(np.std(xs, axis=0) + 1e-8, axis=0, keepdims=True) + 1e-8)
        if self._normalize_outputs:
            # recompute normalizing constants for outputs
            self.model.networks['default'].y_mean.load(
                np.mean(np.mean(ys, axis=0), axis=0, keepdims=True))
            self.model.networks['default'].y_std.load(
                np.std(np.std(ys, axis=0) + 1e-8, axis=0, keepdims=True) + 1e-8)
        if self._use_trust_region:
    ...
    def predict(self, xs):
        ...
        """
        # convert xs into time-series data format
        values = self._f_predict(np.expand_dims(xs, axis=0))
        # convert it back to [time_step, 1]
        values = values.reshape((values.shape[1], -1))
        return values
