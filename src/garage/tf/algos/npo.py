...
class NPO(BatchPolopt):
    ...
    @overrides
    def optimize_policy(self, itr, samples_data, sliced_samples_data=None):
        policy_opt_input_values = self._policy_opt_input_values(samples_data)

        logger.log('Computing loss before')
        loss_before = self.optimizer.loss(policy_opt_input_values)
        logger.log('Computing gradients')
        if sliced_samples_data:
            for task, task_samples_data in enumerate(sliced_samples_data, 1):
                task_opt_input_values = self._policy_opt_input_values(
                    task_samples_data)
                gradient_vars = self.optimizer.gradient(task_opt_input_values)
                for gradient, variable in gradient_vars:
                    tabular.record(
                        'Task{}/{}/{}'.format(task, self.policy.name,
                                              variable), Histogram(gradient))
        else:
            gradient_vars = self.optimizer.gradient(policy_opt_input_values)
            for gradient, variable in gradient_vars:
                tabular.record('{}/{}'.format(self.policy.name, variable),
                               Histogram(gradient))
