...
from garage.tf.regressors import GaussianGRURegressor


class GaussianGRUBaseline(Baseline):
    ...
    def __init__(
            ...
            num_seq_inputs=1,
            regressor_args=None,
            name='GaussianGRUBaseline',
    ):
        super().__init__(env_spec)
        if regressor_args is None:
            regressor_args = dict()
        self._regressor_args = regressor_args
        self._regressor = GaussianGRURegressor(
    ...
    def fit(self, paths):
        ...
        """
        max_len = max([len(p['rewards']) for p in paths])
        observations = np_tensor_utils.stack_and_pad_tensor_n(paths, 'observations', max_len)
        returns = np_tensor_utils.stack_and_pad_tensor_n(paths, 'returns', max_len)
        returns = np.expand_dims(returns, axis=2)
        self._regressor.fit(observations, returns)
