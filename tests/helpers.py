...
def relplot(g_csvs, b_csvs, g_x, g_y, g_z, b_x, b_y, b_z, trials, seeds, plt_file, env_id,
            x_label, y_label):
    """
    ...
    df_gs['Type'] = g_z
    df_bs['Type'] = b_z
    ax = sns.relplot(x=g_x, y=g_y, hue='Type', kind="line", data=pd.concat([df_gs, df_bs]))
    ax.axes.flatten()[0].set_title(env_id)
    plt.savefig(plt_file)

    plt.close()

def plot(g_csvs, b_csvs, g_x, g_y, b_x, b_y, trials, seeds, plt_file, env_id,
         x_label, y_label):
    """
