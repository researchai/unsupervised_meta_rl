...
class MAML(BatchPolopt):
    ...
    def build_inputs(self):
        ...
        for i in range(self.policy.meta_batch_size * 2):
            ...
            if i < self.policy.meta_batch_size:
            ...
            with tf.name_scope("adaptation_loss_{}".format(idx)):
                ...
                with tf.name_scope("policy_loss"):
                    ...
                    with tf.name_scope("surr_loss_{}".format(idx)):
                        loglikili = pol_dist.log_likelihood_sym(
                            i.valid.action_var,
                            policy_dist_info_valid,
                            name="loglikili_{}".format(idx))
                        surr_loss = -tf.reduce_mean(loglikili * adv_valid)
    ...
    def _build_final_loss(self, loss_inputs, kl_inputs, maml_infos):
        ...
        dist_infos_flat = []
        for idx in range(self.policy.meta_batch_size):
    ...
    def policy_opt_input_values(self, samples_data, adaptation_data):
        ...
        for i in range(self.policy.meta_batch_size):
            policy_state_info_list = [
    ...
    def fit_baseline_once(self, samples_data):
        ...
        # TODO remove redundent returns calculations..
        ...
        aug_returns = []
        for ret, val, path in zip(returns_tensor, valids,
                                       paths):
