...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help='Directory of the pickle file to test experiment from.')
    parser.add_argument(
        '--itr',
        type=int,
        default=None,
        help='Iteration of the pickle file to replat with.')
    parser.add_argument(
        '--csv',
        type=str,
        default='test.csv',
    ...
    with tf.Session() as sess:
        ...
        with open(csv_path, 'a+') as csv:
            ...
            for meta_task, meta_path in enumerate(paths):
                for rollout_idx, rollout_path in enumerate(meta_path):
                    rollout_path["returns"] = special.discount_cumsum(rollout_path["rewards"], discount)
                    rollout_task_paths[meta_task]["returns"].append(np.mean(rollout_path["returns"]))
                    rollout_task_paths[meta_task]["rewards"].append(np.sum(rollout_path["rewards"]))
